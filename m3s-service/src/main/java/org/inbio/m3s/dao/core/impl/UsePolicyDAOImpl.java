...
public class UsePolicyDAOImpl extends BaseDAOImpl implements UsePolicyDAO {
	...
	public UsePolicyLite getUsePolicyLite(String name) throws IllegalArgumentException {
		...
		try {
			...
			return new UsePolicyLite(DataCache.usePoliciesDBIds.get(index), name);
		} catch (Exception e) {
	...
	public UsePolicyLite getUsePolicyLite(Integer usePolicyId) throws IllegalArgumentException {
		...
		try {
			...
			return new UsePolicyLite(usePolicyId, DataCache.usePoliciesNames.get(index));
		} catch (Exception e) {
			logger.error(errorMsj);
			logger.error(e.getMessage());
			throw new IllegalArgumentException(errorMsj);
	...
	public List<UsePolicyLite> listAllLite() throws IllegalArgumentException {
		...
		return (List<UsePolicyLite>) template.execute(new HibernateCallback() {
			public Object doInHibernate(Session session) {
				Query query = session.createQuery(
						"select new org.inbio.m3s.dto.lite.UsePolicyLite(up.usePolicyId, tt.name)"
